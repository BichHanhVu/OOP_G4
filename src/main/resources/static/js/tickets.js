...
async function loadTickets() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("Không thể tải dữ liệu phiếu mượn");
    }
...
function filterTickets() {
  ...
  const filteredTickets = allTickets.filter((ticket) => {
    ...
    const overdue =
      ticket.status === "BORROWING" && ticket.dueDate && ticket.dueDate < today;

    // Tìm theo mã phiếu hoặc mã bạn đọc
    const matchesText =
      (ticket.ticketId || "").toLowerCase().includes(keyword) ||
      (ticket.readerId || "").toLowerCase().includes(keyword);

    // Lọc theo trạng thái
    const matchesStatus =
      !status || (status === "OVERDUE" ? overdue : ticket.status === status);
